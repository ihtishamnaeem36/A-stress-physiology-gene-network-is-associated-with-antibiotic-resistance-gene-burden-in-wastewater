...
variable, mostly genus-or-coarser resolution, not species-resolved, despite
...
def main():
    ...
    for s in SAMPLE_COLS_18:
        ...
        top = col.idxmax()
        top_taxon_of[s] = top
        genus_short = top.split('g__')[-1].split('.s__')[0] or "(genus unresolved: " + top.split('.o__')[-1].split('.f__')[0] + ")"
        props = col / col.sum()
        rows.append(dict(
            sample=s,
