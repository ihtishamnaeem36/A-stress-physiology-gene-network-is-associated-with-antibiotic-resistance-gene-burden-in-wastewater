...
# One ARG type (antibacterial_fatty_acid) has zero abundance in every sample and is
# excluded here (constant input -> undefined Spearman correlation), matching the
# manuscript's stated "300 valid tests" (20 types x 15 modules; Section 2.8/3.7).
type_names = [t for t in type_mat.index if type_mat.loc[t, sample_cols].astype(float).nunique() > 1]
excluded_types = [t for t in type_mat.index if t not in type_names]
if excluded_types:
    print(f"Excluded constant-abundance ARG type(s) from the Type x Module matrix: {excluded_types}")
res2 = []
for t in type_names:
